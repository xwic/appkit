 ...
 * 
 */
package de.xwic.appkit.cluster.node.server;
 ...
 *
 */
public class CNodeServer implements Runnable {
	
	private static CNodeServer instance = null;
	
	private static final Log log = LogFactory.getLog(CNodeServer.class);
	 ...
	 * @return
	 */
	public static CNodeServer getInstance() {
		if (instance == null) {
			instance = new CNodeServer();
		}
		return instance;
	}
	...
	private boolean initializeServer() {
		...
		if (FORCE_LOG4J_INITIALIZATION || !Logger.getRootLogger().getAllAppenders().hasMoreElements()) {
			...
			if (isXml) {
				DOMConfigurator.configureAndWatch(configFile.getAbsolutePath(), 2000);
			} else {
				PropertyConfigurator.configureAndWatch(configFile.getAbsolutePath(), 2000);
			}
		}
